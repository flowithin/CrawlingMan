...
std::string HtmlParser::ExtractAttribute(const char *start, const char *end,
  ...
  int left_offset = 1;
  for (; left_ptr < end && IsWordBreak(left_ptr, end, left_offset);
       left_ptr += left_offset) {
  }
  if (left_ptr == end) {
    return "";
  }
  for (; right_ptr < end && *right_ptr != '=' && right_ptr != end;
       right_ptr++) {
  }
  if (right_ptr == end) {
    return "";
...
HtmlParser::HtmlParser(const char *buffer, size_t length)
  ...
  for (pos_ = buffer; pos_ < end_; pos_ = right_ptr) {
    ...
    // right_ptr++) {
    for (right_ptr = pos_ + 1; right_ptr < end_ && IsText(right_ptr, end_);
         right_ptr++) {
    }
    ...
    if (!anchor_stack_.empty() &&
        anchor_stack_.back()->URL == links.back().URL) {
