...
class HtmlParser {
 ...
 private:
  ...
  // Whether currently inside a <title> tag.
  bool in_title_;

  // Whether currently inside ordinary text.
  bool ordinary_text_;

  // Stack of active <a> tags (supports nesting).
  std::vector<Link*> anchor_stack_;

  // Adds word to words/titleWords and anchor text if applicable.
  void AddWord(const std::string& word);

  // Extracts attribute value (e.g., href="..." or src="...") from tag content.
  // Returns empty string if not found.
  std::string ExtractAttribute(const char* start, const char* end, const std::string& attr_name);

  // Skips all content until the specified closing tag is found.
  // Used for script, style, and svg tags.
  void SkipUntilCloseTag(const char* tag_name, size_t tag_len);

  // Skips HTML comment content until --> is found.
  void SkipComment();

  // Processes a single HTML tag at the current position.
  void ProcessTag();
